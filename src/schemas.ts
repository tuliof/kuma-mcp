...
export const MonitorConfigSchema = z
  .object({
    ...
    retryInterval: z.number().default(60).describe('Heartbeat retry interval in seconds (default: 60)'),
    maxretries: z.number().default(0).describe('Maximum retries before the service is marked as down and a notification is sent'),
    resendInterval: z.number().default(0).describe('Resend Notification if Down X times consecutively (default: 0 - no resend)'),
    notificationIDList: z.record(z.string(), z.any()).default({}).describe('Object mapping notification IDs to their settings'),
    ...
    jsonPathOperator: z.enum(['==', '!=', '>', '>=', '<', '<=', 'contains']).optional().describe('Comparison operator for JSON query (for json-query type)'),
    expectedValue: z.string().optional().describe('Expected value for JSON query comparison (for json-query type)'),
    upsideDown: z.boolean().default(false).describe('Flip the status upside down. If the service is reachable, it is DOWN'),
    // HTTP/HTTPS monitor advanced fields
    expiryNotification: z.boolean().default(false).describe('Certificate Expiry Notification - Send notification when TLS certificate expires'),
    ignoreTls: z.boolean().default(false).describe('Ignore TLS/SSL errors for HTTPS websites'),
    uptimeKumaCachebuster: z.boolean().default(false).describe('Add the uptime_kuma_cachebuster parameter - Randomly generated parameter to skip caches'),
    maxredirects: z.number().default(10).describe('Maximum number of redirects to follow. Set to 0 to disable redirects'),
    accepted_statuscodes: z.array(z.string()).default(['200-299']).describe('Accepted Status Codes - Select status codes which are considered as a successful response. Can be ranges (e.g., "200-299") or individual codes (e.g., "200"). Default: ["200-299"]'),
    ipFamily: z.enum(['ipv4', 'ipv6']).nullable().optional().describe('IP Family - Uses the Happy Eyeballs algorithm for determining the IP family. null for auto-select (default), "ipv4" for IPv4 only, "ipv6" for IPv6 only'),
    ...
    packetSize: z.number().default(56).describe('Packet Size - Number of data bytes to be sent (default: 56)'),
    maxPackets: z.number().default(1).describe('Max Packets - Number of packets to send before stopping (default: 1)'),
    numericOutput: z.boolean().default(false).describe('Numeric Output - If checked, IP addresses will be output instead of symbolic hostnames'),
    perPingTimeout: z.number().default(2).describe('Per-Ping Timeout - This is the maximum waiting time (in seconds) before considering a single ping packet lost'),
  })
